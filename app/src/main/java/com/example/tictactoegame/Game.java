...
public class Game extends Activity implements OnClickListener {
    ...
    private static tictactoegame.mod.pyjioh.apps.tictactoe.controllers.TicTacToyControll controller = .pyjioh.apps.tictactoe.controllers.TicTacToyControll
            .getInstance();
    ...
    public void onClick(View v) {
        if (v instanceof Button) {
            doMove((Button) v);
            controller.refreshGame();
            if (model.getState() == TicTacMod.STATE_DRAW)
                showAlertDialog(R.string.draw_game);
            else if (model.getState() == TicTacMod.STATE_WIN) {
                if (model.getWinner() == TicTacMod.NOUGHT)
                    showAlertDialog(R.string.nought_win_game);
                else if (model.getWinner() == TicTacMod.CROSS)
